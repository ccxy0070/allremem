...
import cn.ar.allremem.basics.user.enums.UserStatusEnum;
import cn.ar.allremem.basics.user.enums.UserTypeEnum;
import cn.ar.allremem.basics.user.service.UserSerive;
import cn.ar.allremem.common.page.PageBean;
import cn.ar.allremem.web.base.BaseAction;
...
public class PmsUserAction extends BaseAction {
	...
	public String listPmsUser() {
		try {
			...
			PageBean pageBean = userSerive.listPage(getPageParam(), paramMap);
			this.pushData(pageBean);
			Users pmsUser = getLoginedUser();// 获取当前登录用户对象
	...
	public String viewPmsUserUI() {
		try {
			...
			Users pmsUser = userSerive.getById(userId);
			if (pmsUser == null) {
	...
	public String addPmsUser() {
		try {
			...
			// 校验用户登录名是否已存在
			Users userNoCheck = userSerive.findUserByUserNo(userNo);
			if (userNoCheck != null) {
	 ...
	 * @return
	 */
	private String validatePmsUser(Users user) {
		 ...
		 * !ValidateUtils.isAlphanumeric(loginPwd)) { loginPwdMsg += "登录密码应为字母或数字组成，"; }
		 */
		msg += userPwdMsg;
	...
	public String editPmsUserUI() {
		try {
			Long id = getLong("id");
			Users pmsUser = userSerive.getById(id);
			if (pmsUser == null) {
				return operateError("无法获取要修改的数据");
			}

			// 普通用户没有修改超级管理员的权限
			if (UserTypeEnum.ADMIN.getValue().equals(this.getLoginedUser().getUserType())
					&& UserTypeEnum.ADMIN.getValue().equals(pmsUser.getUserType())) {
	...
	public String changeUserStatus() {
		try {
			Long userId = getLong("id");
			Users user = userSerive.getById(userId);
			if (user == null) {
	...
	public String resetUserPwdUI() {
		Users user = userSerive.getById(getLong("id"));
		if (user == null) {
	...
	public String resetUserPwd() {
		try {
			...
			userSerive.updateUserPwd(userId, DigestUtils.sha1Hex(newPwd), false);
	...
	public String userChangeOwnPwd() {
		try {
			...
			Users user = this.getLoginedUser();
			if (user == null) {
			...
			// 更新密码
			userSerive.updateUserPwd(user.getId(), DigestUtils.sha1Hex(newPwd), true);
	...
	public String userViewOwnInfo() {
		try {
			...
			Users pmsUser = this.getLoginedUser();
			if (pmsUser == null) {
				return operateError("无法从会话中获取用户信息");
			}

			Users user = userSerive.getById(pmsUser.getId());
			if (user == null) {
